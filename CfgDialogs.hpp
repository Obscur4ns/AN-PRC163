...
class UKSF_PRC163_RadioDialog {
    ...
    class controls {
        class VolumeKnob: UKSF_PRC163_RscHotspot {
            ...
            w = PRC163_W(100);
            h = PRC163_H(138);
            onMouseButtonUp = "private _direction = if ((_this select 1) isEqualTo 1) then {-1} else {1}; private _radio = uiNamespace getVariable ['UKSF_PRC163_guiRadio','']; if (_radio isEqualTo '') then {_radio = [] call UKSF_PRC163_fnc_getTargetRadio;}; if !(_radio isEqualTo '') then {private _pair = [_radio,player,false] call UKSF_PRC163_fnc_resolvePair; private _radioA = _pair param [0,'']; private _radioB = _pair param [1,'']; if !(_radioA isEqualTo '' || {_radioB isEqualTo ''}) then {private _line = [_radioA,'getState','prc163SelectedLine'] call acre_sys_data_fnc_dataEvent; if !(_line in [0,1]) then {_line = 0;}; private _targetRadio = [_radioA,_radioB] select _line; private _stateName = ['prc163VolumeA','prc163VolumeB'] select _line; private _value = [_radioA,'getState',_stateName] call acre_sys_data_fnc_dataEvent; if !(_value isEqualType 0) then {_value = 0.8;}; _value = ((_value + (0.1 * _direction)) max 0) min 1; [_targetRadio,'setVolume',_value] call acre_sys_data_fnc_dataEvent;};};";
