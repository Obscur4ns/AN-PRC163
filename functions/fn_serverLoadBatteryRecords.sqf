...
if (_version isEqualTo 1) then {
    ...
    if (_playerIndex >= 0) then {
        _records = (_players select _playerIndex) param [1,[],[[]]];
    };
};

if (local _unit && {hasInterface}) then {
    [_records] call UKSF_PRC163_fnc_receiveBatteryRecords;
} else {
    [_records] remoteExecCall [
        "UKSF_PRC163_fnc_receiveBatteryRecords",
        owner _unit
    ];
};
